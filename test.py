...
def check_new_time(
    dbname, dbuser, url=DEFAULT_AVITO_URL, count_p=2
):  # Передаем, ссылку и количество страниц указанное пользователем
    print("start storoj")
    conn = psycopg2.connect("dbname=" + dbname + " user=" + dbuser)
    cur = conn.cursor()

    source_dict = {}
    source_avito = get_avito(
        url, count_p
    )  # Первый эталонный запрос(источник для парсинга)
    source_dict.update(source_avito)
    for oneString in source_dict.items():
        # print(oneString)
        dateplustime = " ".join(oneString[1][3])
        cur.execute(
            "INSERT INTO ads (id_ads, adname, price, city, url, addate) VALUES (%s, %s, %s, %s, %s, %s);",
            (
                oneString[0],
                oneString[1][0],
                oneString[1][1],
    ...
    while True:
        ...
        check_avito = get_avito(
            url, count_p
        )  # Новый запрос "Проверка новых объявлений"
        ...
        for i in check_dict.items():
            ...
            if check_new_date == today:
                ...
                check_new_time = i[1][3][
                    1
                ]  # Если ошибка out of range, смотреть в parser_p 69 строка
                ...
                if sec >= totime_sec - 600:
                    ...
                    if i not in source_dict.items():
                        ...
                        cur.execute(
                            """INSERT INTO ads (id_ads, adname, price, city, url, addate) 
                            VALUES (%s, %s, %s, %s, %s, %s);""",
                            (i[0], i[1][0], i[1][1], i[1][2], i[1][4], dateplustime),
                        )
                        source_dict[i[0]] = i[1]
