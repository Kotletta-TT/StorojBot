...
import psycopg2
import test
from func import get_proxy, get_token as proxy, token
...
def send_welcome(message):
    ...
    bot.reply_to(
        message,
        """Привет, я бот-сторож для Авито, я буду "сторожить" авито и если что
         то появится по твоей 
	теме, я тебя оповещу и ты сможешь первым отреагировать на новые объявления""",
    )
    time.sleep(3)
    bot.send_message(
        chatid,
        """просто отправь /url "ссылка на товар или группу товаров в твоем 
        городе или по россии" """,
    )
...
def start_url(message):
    ...
    bot.send_message(
        chatid,
        """Я просканирую первые страницы и буду ждать новых объявлений, 
        в случае появления - я вам все пришлю""",
    )
    global t1
    ...
    t1.join()
    bot.send_message(
        chatid, "Завершаю работу, чищу базу. Если что-то понадобится... только скажи"
    )
    conn = psycopg2.connect("dbname=" + dbname + " user=" + dbuser)
    cur = conn.cursor()
    cur.execute("DROP TABLE ads;")
    cur.execute(
        """CREATE TABLE ads (id serial primary key, id_ads integer, adname text, 
        price text, city text, url text, addate text);"""
    )
    conn.commit()
    conn.close()
