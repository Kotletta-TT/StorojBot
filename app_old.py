...
def index():
    ...
    if request.method == "POST":
        ...
        if "callback_query" in dat_json.keys():
            ...
            if "storoj_bot" == data:
                # parser_buttons = [{"text":"URL","callback_data":"url_pars"},
                       # {"text":"Запрос","callback_data":"zapros_pars"}]
                func.send_Message(
                    chat_id,
                    """Запуск сторожа. Введите URL-ссылку на Авито, 
                    либо запрос (учтите запрос идет по дефолтной ссылке 
                    авито - по Всей России, по всем категориям)""",
                )
            if "auto_load" == data:
                storoj_buttons = [
                    {"text": "URL", "callback_data": "url_pars"},
                    {"text": "Запрос", "callback_data": "zapros_storoj"},
                ]
                func.editMessageText(
                    chat_id, "Запуск Автозагрузки", message_id, storoj_buttons
                )
        else:
