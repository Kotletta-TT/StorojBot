...
def parser_pages(
    url="https://www.avito.ru/sochi/tovary_dlya_kompyutera/
    komplektuyuschie/videokarty?s_trg=11&cd=1",
    count_pages=2,
):
    data_dict = {}
    request = requests.get(url, timeout=5)
    soup = BeautifulSoup(request.content, "lxml")
    # Пока не могу нормально получить количество страниц с авито, 
    # поэтому данная часть закоментированна
    ...
    for count_p in range(count_pages):  # ads_count_pages):
        ...
        for container in ads_container:
            ...
            try:
                ads_name = (
                    container.find("a", {"class": "snippet-link"})
                    .get_text()
                    .split("\n")[1]
                )
            ...
            try:
                ...
                try:
                    ads_city = (
                        container.find("span", {"class": "item-address__string"})
                        .get_text()
                        .split("\n")[1]
                    )
                    pre_data_list.append(ads_city)
            ...
            try:
                ...
                current_time = full_bugs_time.split(
                    " "
                )  # если не разделяет время, вместо пробела поставить \xa0
